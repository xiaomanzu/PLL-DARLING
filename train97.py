...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset == 'PACS':
        train_loader, train_givenY, train_sampler, test_loader,trainloader,domain_label,domain_number = load_PACS(partial_rate=args.partial_rate, batch_size=args.batch_size)
        # input_size = 224  # fixed as 224
        # train_loader, train_givenY, train_sampler, test_loader = load_PACS(input_size=input_size
        #     , partial_rate=args.partial_rate, batch_size=args.batch_size)
    elif args.dataset == 'VLCS':
        train_loader, train_givenY, train_sampler, test_loader = load_VLCS(partial_rate=args.partial_rate, batch_size=args.batch_size)
    else:
    ...
    criterion=PLLLoss(domain_number)
    print('1')
    for i,(input,label) in enumerate(trainloader):
        print('2')
        feature=model(input)
        print('3')
        loss=(feature,domain_label)
